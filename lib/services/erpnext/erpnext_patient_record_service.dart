...
class ErpNextPatientRecordService implements PatientRecordService {
  ...
  Future<Result<List<MedicalRecord>>> getRecords(String patientId) async {
    try {
      // Scoped server endpoint resolves the patient from the logged-in user,
      // so the patient's own login can read its encounters (no doctype perms).
      final response =
          await _api.dio.post('/method/hiraal_emr.api.get_my_records');
