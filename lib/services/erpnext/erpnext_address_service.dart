...
class ErpNextAddressService implements AddressService {
  ...
  Future<Result<List<PatientAddress>>> getAddresses(String patientId) async {
    try {
      // Scoped server endpoint returns the logged-in patient's own addresses.
      final response =
          await _api.dio.post('/method/hiraal_emr.api.get_my_addresses');

      final list = response.data?['message'] as List? ?? [];
      final addresses = list
          .cast<Map<String, dynamic>>()
          .map(PatientAddress.fromJson)
          .toList();
  ...
  Future<Result<PatientAddress>> addAddress(
    ...
    try {
      final response = await _api.dio.post(
        '/method/hiraal_emr.api.add_my_address',
        data: {
          'label': address.label,
          'address_type': _mapAddressType(address.type),
          'address_line1': address.address,
          'is_primary': address.isDefault ? 1 : 0,
        },
      );
  ...
  Future<Result<void>> deleteAddress(String addressId) async {
    try {
      await _api.dio.post(
        '/method/hiraal_emr.api.delete_my_address',
        data: {'name': addressId},
      );
