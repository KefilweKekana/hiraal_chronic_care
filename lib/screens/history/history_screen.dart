...
class _HistoryScreenState extends State<HistoryScreen> with SingleTickerProviderStateMixin {
  ...
  Map<int?, String> _filterOptions(AppLocalizations l10n) => {
        null: l10n.filterAllTime,
        7: l10n.filterLast7Days,
        30: l10n.filterLast30Days,
        90: l10n.filterLast90Days,
      };
                  ...
                  Flexible(
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            filterOptions[_filterDays] ?? l10n.filter,
                            maxLines: 2,
            ...
            Container(
              ...
              child: TabBar(
                ...
                tabs: [
                  ...
                  Tab(child: Text(l10n.tabAlerts, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis)),
                ],
  ...
  Widget _buildReadingsTab(List<VitalReading> readings) {
    // Nothing cached yet while the provider is busy: show shimmer placeholders
    // shaped like the list/chart instead of an empty area.
    final provider = context.watch<AppProvider>();
    final l10n = AppLocalizations.of(context);
    final loading = provider.isLoading && provider.readings.isEmpty;
    return Column(
      children: [
        // List / Chart toggle
        Padding(
        ...
        Expanded(
          child: loading
              ? (_isChartView ? _buildChartSkeleton() : const SkeletonList(itemCount: 6))
              : _isChartView
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (DateFormat('yyyy-MM-dd').format(readingDate) == DateFormat('yyyy-MM-dd').format(now)) {
          dateLabel = l10n.todayDateLabel(dateKey);
        } else if (DateFormat('yyyy-MM-dd').format(readingDate) ==
            DateFormat('yyyy-MM-dd').format(now.subtract(const Duration(days: 1)))) {
          dateLabel = l10n.yesterdayDateLabel(dateKey);
        } else {
          dateLabel = dateKey;
        }
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              _ChartLegendItem(color: AppColors.error, label: l10n.systolic),
              const SizedBox(width: 16),
              _ChartLegendItem(color: AppColors.chartBlue, label: l10n.diastolic),
              const SizedBox(width: 16),
              _ChartLegendItem(color: AppColors.warning, label: l10n.sugar),
            ],
          ),
  ...
  /// Shimmer placeholder roughly matching the chart layout (legend + plot area).
  ...
  Widget _buildAlertsTab(List<VitalReading> readings) {
    ...
    final alerts = readings.where((r) {
      return (r.systolic ?? 0) >= 140 || (r.bloodSugar ?? 0) >= 200;
    }).toList();
    if (alerts.isEmpty) {
      return Center(
...
class _ReadingListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final statusText = reading.status == 'Sent'
        ? l10n.statusSent
        : (reading.status ?? l10n.statusPending);
    return Container(
