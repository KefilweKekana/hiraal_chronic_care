...
class _SyncScreenState extends State<SyncScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Sync in Progress'), leading: IconButton(icon: Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context))),
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Container(
              width: 64, height: 64,
              decoration: BoxDecoration(color: AppColors.of(context).primarySoft, borderRadius: BorderRadius.circular(16)),
            ...
            Text("We're syncing your saved readings to\nyour health record.", textAlign: TextAlign.center, style: TextStyle(color: AppColors.of(context).textMuted)),
            const SizedBox(height: 24),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Sync Progress', style: TextStyle(fontWeight: FontWeight.w600)),
                    Text('${_items.where((i) => i['sent'] == true).length} of ${_items.length} Items', style: TextStyle(color: AppColors.of(context).textMuted)),
              ],
            ),
            const SizedBox(height: 8),
            ClipRRect(
              borderRadius: BorderRadius.circular(8),
              child: LinearProgressIndicator(value: _progress, minHeight: 8, backgroundColor: AppColors.of(context).inputFill, color: AppColors.primary),
            ),
            Text('${(_progress * 100).toInt()}%', style: TextStyle(fontSize: 12, color: AppColors.primary)),
            const SizedBox(height: 20),
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(item['type'] as String, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                      Text(item['detail'] as String, style: TextStyle(fontSize: 11, color: AppColors.of(context).textMuted)),
                    ])),
                    if (sent) Icon(Icons.check_circle, color: AppColors.success, size: 20)
                    else const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.primary)),
                    const SizedBox(width: 4),
                    Text(sent ? 'Sent' : 'Sending...', style: TextStyle(fontSize: 11, color: sent ? AppColors.success : AppColors.of(context).textFaint, fontWeight: FontWeight.w600)),
                  ],
                ),
              );
            }),
            ...
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.of(context).successSoft, borderRadius: BorderRadius.circular(10)),
              child: const Row(
                children: [
  ...
  Widget _buildUpToDateView(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.of(context).scaffold,
      appBar: AppBar(title: const Text('Sync'), leading: IconButton(icon: Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context))),
      body: Padding(
        padding: const EdgeInsets.all(24),
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Icon(Icons.cloud_done, color: AppColors.white, size: 40),
            ),
            const SizedBox(height: 16),
            const Text("You're Up to Date", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text('No saved readings are waiting to sync.\nEverything is already on your health record.', textAlign: TextAlign.center, style: TextStyle(color: AppColors.of(context).textMuted)),
            const Spacer(),
            SizedBox(
  ...
  Widget _buildCompleteView(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppColors.of(context).scaffold,
      appBar: AppBar(title: const Text('Sync Complete'), leading: IconButton(icon: Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context))),
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            Text(allSent ? "You're All Set!" : 'Sync Incomplete', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text(
              allSent
            ...
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: allSent ? AppColors.of(context).infoSoft : AppColors.of(context).warningSoft, borderRadius: BorderRadius.circular(10)),
              child: Row(
                children: [
            ...
            Align(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(allSent ? 'What Was Synced' : 'Sync Results', style: TextStyle(fontWeight: FontWeight.w600)),
                    Text('$sentCount of ${_items.length} Items', style: TextStyle(color: AppColors.of(context).textMuted, fontSize: 13)),
                ],
              ),
            ),
            const SizedBox(height: 12),
            ...
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: allSent ? AppColors.of(context).successSoft : AppColors.of(context).warningSoft, borderRadius: BorderRadius.circular(10)),
              child: Row(
                children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.of(context).border)),
              child: Row(
                children: [
                  Icon(Icons.sync, size: 18, color: AppColors.of(context).textMuted),
                  const SizedBox(width: 8),
                  Expanded(child: Text("Next sync\nWe'll automatically sync new data\nwhen you go offline again.", style: TextStyle(fontSize: 12, color: AppColors.of(context).textMuted))),
