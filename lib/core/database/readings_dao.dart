...
class ReadingsDao {
  ...
  /// Whether a reading with this app-side reference id already exists —
  /// lets the BLE auto-submit skip re-inserted store-and-forward readings.
  Future<bool> existsByReference(String referenceId) async {
    if (!_supported || referenceId.isEmpty) return false;
    final db = await _dbHelper.database;
    final result = await db.rawQuery(
      'SELECT COUNT(*) as cnt FROM readings WHERE reference_id = ?',
      [referenceId],
    );
  ...
  Future<void> markSynced(int localId, {String? serverId}) async {
    ...
    // The queue only tracks not-yet-synced readings; drop the row on success.
    await db.delete(
      'sync_queue',
      where: "table_name = 'readings' AND record_id = ?",
      whereArgs: [localId],
    );
  }

  /// Mark a reading as synced by its app-side reference id. Used after a
  /// successful server submit so the background sync never re-sends it.
  Future<void> markSyncedByReference(String? referenceId, {String? serverId}) async {
    if (!_supported || referenceId == null || referenceId.isEmpty) return;
    final db = await _dbHelper.database;
    await db.update(
      'readings',
      {
    ...
    await db.delete(
      'sync_queue',
      where: "table_name = 'readings' AND record_id IN "
          '(SELECT local_id FROM readings WHERE reference_id = ?)',
      whereArgs: [referenceId],
    );
  }
