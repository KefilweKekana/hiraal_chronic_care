...
class DatabaseHelper {
  ...
  Future<Database> get database async {
    ...
    // Cache the in-flight open so concurrent getters share one connection
    // instead of racing to open (and leaking) two.
    _dbFuture ??= _initDatabase();
    try {
      _database = await _dbFuture!;
    } catch (_) {
      _dbFuture = null; // allow a later call to retry after a failed open
      rethrow;
    }
    return _database!;
  }
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      // The paywall gate reads the last-known subscription state on session
      // restore; without this column every restore defaulted to "no
      // subscription" and flashed (or stuck) the paywall for paying patients.
