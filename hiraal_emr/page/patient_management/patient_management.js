(function () {
// ─── Patient Management — rendered inside the shared HiraalShell (same
...
class PatientManagement {
  ...
  shell(content) {
    this.container.html(HiraalShell.render("patients", "Patient Management", content));
    HiraalShell.bind(this.container, () => this.load_data());
  }

  async load_data() {
    this.shell('<div class="hd-loading">Loading patients…</div>');
    try {
      ...
      console.error(e);
      this.shell('<div class="hd-loading">Error loading patient data.</div>');
    }
  }
